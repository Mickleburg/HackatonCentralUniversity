...
"""
Run NER inference on TSV dataset and save predictions.

Reads TSV with at least 'text' column, runs batch prediction, and saves:
...
def load_tsv(input_path: Path) -> pd.DataFrame:
    """Load TSV file and validate required columns."""
    ...
    try:
        ...
        raise ValueError(f"Failed to read TSV file '{input_path}': {exc}") from exc
...
def prepare_texts(df: pd.DataFrame) -> List[str]:
    """Extract and normalize texts from dataframe."""
    texts: List[str] = []
...
def format_predictions_as_tuple_string(predictions: List[Prediction]) -> str:
    """
    Format predictions as a Python-like list of tuples:
    [(start, end, "CATEGORY"), ...]
    """
    if not predictions:
        return "[]"
...
def save_predictions_tsv(
    ...
    """
    Save predictions to TSV with original columns plus `predictions`.

    `predictions` is formatted as:
    [(start, end, "CATEGORY"), ...]
    """
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run NER inference on TSV dataset")
    parser.add_argument(
        "--input",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional TSV output path with predictions as [(start, end, \"CATEGORY\"), ...]. "
             "If omitted, derived automatically from --output.",
    )
    parser.add_argument(
        "--model_dir",
        type=str,
    ...
    if len(all_predictions) != len(texts):
        print(
            f"❌ ERROR: Prediction count mismatch: "
            f"{len(all_predictions)} predictions for {len(texts)} texts"
        )
        sys.exit(1)
